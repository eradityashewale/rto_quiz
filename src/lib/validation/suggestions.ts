...
export const MAX_SUGGESTION_ATTACHMENTS = 3;
export const MAX_SUGGESTION_ATTACHMENT_BYTES = 2 * 1024 * 1024; // 2MB decoded per image
...
const attachmentSchema = z
  ...
  .refine(
    (val) => {
      ...
      return bytes <= MAX_SUGGESTION_ATTACHMENT_BYTES;
    },
    { message: "Image is too large (max 2MB)" }
  );

const attachmentsSchema = z.array(attachmentSchema).max(MAX_SUGGESTION_ATTACHMENTS).optional().default([]);
...
export const addSuggestionMessageSchema = z
  .object({
...
export type AddSuggestionMessageInput = z.infer<typeof addSuggestionMessageSchema>;
