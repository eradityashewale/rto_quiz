import { MAX_SUGGESTION_ATTACHMENTS, MAX_SUGGESTION_ATTACHMENT_BYTES } from "@/lib/validation/suggestions";

export { MAX_SUGGESTION_ATTACHMENTS, MAX_SUGGESTION_ATTACHMENT_BYTES };
...
export async function processImageFiles(
  ...
  for (const file of Array.from(files)) {
    if (accepted.length >= remaining) {
      rejected.push(`${file.name}: too many images (max ${MAX_SUGGESTION_ATTACHMENTS})`);
      continue;
    }
