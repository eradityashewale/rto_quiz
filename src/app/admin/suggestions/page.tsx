...
type FilterTab = "ALL" | "OPEN" | "REVIEWED" | "CLOSED";
...
export default function AdminSuggestionsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/suggestions${qs}`)
      .then(async (res) => {
        if (res.status === 401) {
  ...
  return (
    <main className="mx-auto max-w-5xl px-4 py-10 sm:px-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">User Suggestions</h1>
          <p className="mt-1 text-slate-600">Feature ideas and feedback raised by users.</p>
        </div>
              ...
              <li key={suggestion.id}>
                ...
                >
                  ...
                  <p className="mt-1 text-sm text-slate-600">
                    <span className="font-medium text-slate-700">{suggestion.userName}</span> — {suggestion.lastMessage}
                  </p>
